...
# Convert healthy to TRUE and sick to FALSE
vertebralcolumn[ ,'class'] <- as.factor(vertebralcolumn[ ,'class']=='NO')
...
healthy <- vertebralcolumn[vertebralcolumn[ ,'class'] == T, ]
unhealthy <- vertebralcolumn[vertebralcolumn[ ,'class'] == F, ]
...
# Select tree size that minimizes deviance
J <- t2$size[t2$dev == min(t2$dev)]
J <- min(J)

# Prune tree based on misclassification error
